    ...
    return (
        <div className="h-full flex flex-col glass-dark">
            ...
            <div className="px-4 py-3 border-b border-white/5 flex items-center justify-between bg-dark-800/50">
                ...
                {hasRun && (
                    <span
                        className={`text-xs font-bold font-mono ${allPassed ? 'text-accent-green' : 'text-red-400'}`}
                    >
                ...
                {!hasRun ? (
                    <div className="h-full flex items-center justify-center">
                        <div className="text-center text-slate-500">
                            ...
                            <p className="text-sm font-medium text-slate-400 mb-1">Ready to test your code</p>
                            <p className="text-xs text-slate-600">Click "Run Code" when you're ready</p>
                        </div>
                    </div>
                ) : allPassed ? (
    ...
    return (
        ...
        >
            ...
            <div className="flex items-center gap-2 mb-2">
                ...
                <span
                    className={`text-sm font-medium ${result.passed ? 'text-accent-green' : 'text-red-400'}`}
                >
            ...
            {/* Test Description */}
            {result.description && (
                <p className={`text-xs mb-2 ${result.passed ? 'text-accent-green/70' : 'text-red-400/70'}`}>
                    {result.description}
                </p>
            )}
            ...
            {result.error && (
                <div className="bg-red-500/10 border border-red-500/30 rounded-lg px-3 py-2 mb-3">
                    <p className="text-xs font-mono text-red-400">Error: {result.error}</p>
                </div>
            )}
            ...
            {result.consoleLogs && result.consoleLogs.length > 0 && (
                <div className="bg-dark-800/80 border border-white/10 rounded-lg px-3 py-2 mb-3">
                    <div className="text-[10px] font-medium text-slate-500 uppercase tracking-wider mb-1.5">Console Output</div>
            ...
            <div className="space-y-1 font-mono text-xs mt-3 p-3 bg-dark-900/50 rounded-lg">
                ...
                <div className="flex">
                    <span className="text-slate-500 w-20">Received:</span>
                    <span className={result.passed ? 'text-accent-green' : 'text-red-400'}>
