    ...
    return (
        <header className={`md:hidden fixed top-0 left-0 right-0 z-40 border-b px-4 py-3 flex items-center justify-between ${theme === 'dark'
            ...
            <button
                ...
                className={`p-2 -ml-2 rounded-lg transition-colors touch-target ${theme === 'dark' ? 'hover:bg-dark-500/50' : 'hover:bg-slate-100'
                    }`}
                aria-label={isMenuOpen ? 'Close menu' : 'Open menu'}
            >
                <svg
                    className={`w-6 h-6 ${theme === 'dark' ? 'text-slate-300' : 'text-slate-700'
                        }`}
                    fill="none"
                    stroke="currentColor"
                    viewBox="0 0 24 24"
